...
public class VertexTransformerTransparency extends VertexTransformer {
    ...
    public void put(int element, float... data) {
	    if (getVertexFormat().getElement(element).getUsage() == VertexFormatElement.Usage.COLOR) {
	    ...
	    if (getVertexFormat().getElement(element).getUsage() == VertexFormatElement.Usage.POSITION) {
            float[] newData = new float[data.length];
            for (int i = 0; i < data.length; i++)
                newData[i] = data[i];
